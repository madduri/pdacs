...
int load_density_field(int fileno, int *TotNumPart, char base[], int total_local_size, fftw_real* dens_in); 
int load_catalog(struct particle_data **P, float BoxSize, int *NumPart, int *TotNumPart, char input_fname[]);
int load_haloes(int *NumPartTotal, float BoxSize, struct particle_data **P); 
...
int butterworth(float flo, float fhi, int order, int num_time_pts, float deltafreq, double complex *fltr); 
int grid_dist_ngp(int n, int NumPart, int TotNumPart, float BoxSize, struct particle_data *P, int local_nx, int local_x_start, fftw_real *dens_in);
...
int grid_dist_cic_mpi_2(int rank, int nproc, int n, int NumPart, int TotNumPart, float BoxSize, float mass, struct particle_data *P, 
		      int local_nx, int local_x_start,fftw_real *dens_in); 
int grid_dist_cic_mpi_rand_2(int rank, int nproc, int n, int NumPart, int *NumPartSub, struct particle_data *P, 
			     int local_nx, int local_x_start,fftw_real *dens_in); 
...
int grid_dist_tsc_mpi_2(int rank, int nproc, int n, int NumPart, int TotNumPart, float BoxSize, float mass, struct particle_data *P, 
			int local_nx, int local_x_start,fftw_real *dens_in);
int grid_dist_tsc_mpi_3(int rank, int nproc, int n, int NumPart, int TotNumPart, float BoxSize, float mass, struct particle_data *P, 
			int local_nx, int local_x_start,fftw_real *dens_in);
...
int get_pk(int ngrid, int NumPart, int local_ny, int local_y_start, float BoxSize, fftw_complex *dens_out, double *dpower,int *ipower, double *kpower); 
...
int get_pk_para_perp(int ngrid, int NumPart, int nmu, float *theta, int local_ny, int local_y_start, float BoxSize, 
		     fftw_complex *dens_out, int los, double *dpower, int *ipower);
int get_pk_legendre(int ngrid, int NumPart, int local_ny, int local_y_start, float BoxSize, fftw_complex *dens_out, int los, 
		    double *dpower_mono, double *dpower_quad, int *ipower); 
int get_pk_new(int rank, int nproc, int n, double TotNumPart, int local_y_start_after_transpose, int local_ny_after_transpose, float BoxSize, fftw_complex*dgrid, char filename[]); 
int get_pk_new_redshift(int rank, int nproc, int n, int nmu, int NumPart, int local_y_start_after_transpose, int local_ny_after_transpose, float BoxSize, int los, fftw_complex*dgrid, double *kmodes, double *kmodes_para, double *kmodes_perp, double *pk, double *pk_mono, double *pk_quad, int *lcnt, int *lcnt_para, int *lcnt_perp, int *lcnt_multipole); 
int calc_delta2(int rank, int nproc, int n, int NumPart, int local_y_start_after_transpose, int local_ny_after_transpose, float BoxSize, fftw_complex*dgrid); 
int get_xi_new(int ngrid, int local_x_start, int local_nx, fftw_real *dens_in, double *kpower_local, double *dpower_local, int *ipower_local); 
int get_xi_new_redshift(int ngrid, int nmu, int local_x_start, int local_nx, int los, fftw_real *dens_in, double *kmodes, double *kmodes_para, double *kmodes_perp, 
			double *pk, double *pk_mono, double *pk_quad, int *lcnt, int *lcnt_para, int *lcnt_perp, int *lcnt_multipole); 
int redshift_space(int NumPart, int los, float redshift, float w, float Omega0, float BoxSize, struct particle_data *P);
int read_parameters(int model_no, float *Om, float *ns, float *w, float *sigma8, float *h); 
...
int load_hacc(int nfiles, int fileno, char input_fname[], int *NumPart, int *TotNumPart, struct particle_data **P);
...
int main(int argc, char **argv)
{
    ...
    char input_fname[200], output_fname[200], output_fname1[200], output_fname2[200];    
    sprintf(input_fname, "%s", argv[2]); 
    int files = (int)atoi(argv[3]);
    sprintf(output_fname, "%s", argv[4]); 
    float BoxSize = (float)atof(argv[5]);
 
    int NumPart = 0, TotNumPart=0;
    FILE *fp, *fout, *fout1, *fout2;
    ...
    double *kpower,*kpower_local; 
    int i,j,k; 

    fftw_complex *dens_out;
    fftw_real *dens_in, *work;
    rfftwnd_mpi_plan p, ip;
	...
	if (argc > 6)
	{
	    ...
	    float w0 = (float)atof(argv[7]); 
	    float Omega0 = (float)atof(argv[8]); 
	    float any[4]; 
	    redshift_space(NumPart, los, redshift, w0, Omega0, BoxSize, Psurvey); /* convert to redshift space */

	}

	if (i==0) /*First time through*/
	    setup_grid(ngrid, local_nx, dens_in);
...
/*     TotNumPart = NumPartSub;  */
    double rhobar = TotNumPart/((double)ngrid*(double)ngrid*(double)ngrid); 

    for (i=0; i < total_local_size; i++)
      {
	dens_in[i]=dens_in[i]/rhobar-1.;
/* 	dens_in_zspace[i]=dens_in_zspace[i]/rhobar-1.; */
      }
    ...
    if (argc > 6)
    {
	int nmu = ngrid; // this should always be the case for k_perp and k_para

	dpower_local = calloc(ngrid*nmu,sizeof(double)); dpower = calloc(ngrid*nmu,sizeof(double));
	ipower_local = calloc(ngrid*nmu,sizeof(int)); ipower = calloc(ngrid*nmu,sizeof(int));
	double *kpower_para_local = calloc(ngrid,sizeof(double)); double *kpower_para = calloc(ngrid,sizeof(double)); 
	double *kpower_perp_local = calloc(ngrid,sizeof(double)); double *kpower_perp = calloc(ngrid,sizeof(double)); 


	double *dpower_mono_local = calloc(ngrid,sizeof(double)); double *dpower_mono = calloc(ngrid,sizeof(double)); 
	double *dpower_quad_local = calloc(ngrid,sizeof(double)); double *dpower_quad = calloc(ngrid,sizeof(double)); 
	kpower_local = calloc(ngrid,sizeof(double)); kpower = calloc(ngrid,sizeof(double)); 
	int *ipower_multi_local = calloc(ngrid,sizeof(int)); int *ipower_multi = calloc(ngrid,sizeof(int));
	int *ipower_para_local = calloc(ngrid,sizeof(int)); int *ipower_para = calloc(ngrid,sizeof(int));
	int *ipower_perp_local = calloc(ngrid,sizeof(int)); int *ipower_perp = calloc(ngrid,sizeof(int));
	Pk = calloc(ngrid*nmu,sizeof(double));

	calc_delta2(rank, numprocs, ngrid, TotNumPart, local_y_start_after_transpose, local_ny_after_transpose, BoxSize, dens_out); 

	rfftwnd_mpi(ip, 1, dens_in, work, FFTW_TRANSPOSED_ORDER);

	get_xi_new_redshift(ngrid, nmu, local_x_start, local_nx, los, dens_in, kpower_local, kpower_para_local, kpower_perp_local, dpower_local, dpower_mono_local, dpower_quad_local,
			    ipower_local, ipower_para_local, ipower_perp_local, ipower_multi_local);



	MPI_Allreduce(kpower_perp_local, kpower_perp, ngrid, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(kpower_para_local, kpower_para, ngrid, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(dpower_local, dpower, ngrid*nmu, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(ipower_local, ipower, ngrid*nmu, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

	MPI_Allreduce(kpower_local, kpower, ngrid, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(dpower_mono_local, dpower_mono, ngrid, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(dpower_quad_local, dpower_quad, ngrid, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(ipower_multi_local, ipower_multi, ngrid, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(ipower_para_local, ipower_para, ngrid, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(ipower_perp_local, ipower_perp, ngrid, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	...
	if (rank == 0)
	  {
	    ...
	    if(!(fout1 = fopen(output_fname1, "w")))
	      {
		printf("can't open file `%s`\n",output_fname1);
		exit(0);
	      }

	    if(!(fout2 = fopen(output_fname2, "w")))
	      {
		printf("can't open file `%s`\n",output_fname2);
		exit(0);
	      }
	    ...
	    for (i = 0; i < ngrid; i++) /*Don't include DC mode*/
	      {

		kpower_perp[i] /= ipower_perp[i];
		kpower_para[i] /= ipower_para[i];
		dpower_mono[i]/=ipower_multi[i];
		dpower_quad[i]/=ipower_multi[i];
		kpower[i] /= ipower_multi[i];
		if (ipower_multi[i] != 0)
		  fprintf(fout1, "%f %f %f \n", kpower[i]*(BoxSize)/(float)ngrid, dpower_mono[i], dpower_quad[i]);
	      }

	    for (i = 0; i < ngrid; i++) /*Don't include DC mode*/
	      if(ipower_perp[i] != 0 && ipower_para[i]!=0)
		{
		  for (j = 0; j< nmu; j++)
		    {
		      if (ipower[j+nmu*i]!=0)
			{
			  dpower[j+nmu*i]/=ipower[j+nmu*i];
			  float mu = kpower_para[j]/sqrt(kpower_perp[i]*kpower_perp[i]+kpower_para[j]*kpower_para[j]);
			  fprintf(fout2, "%f %f %f %f \n", kpower_para[j]*BoxSize/(float)ngrid, kpower_perp[i]*BoxSize/(float)ngrid, mu, dpower[j+nmu*i]);
			}
		    }
		}
	...
	free(dpower_mono_local); 
	free(dpower_quad_local); 
	free(dpower_mono);
	free(dpower_quad);
	free(ipower_multi_local);
	free(ipower_multi); 
	free(dpower);
	free(ipower);
	free(dpower_local);      
	...
	free(Pk);
      ...
      {
	dpower_local = calloc(2*ngrid,sizeof(double)); dpower = calloc(2*ngrid,sizeof(double)); 
	kpower_local = calloc(2*ngrid,sizeof(double)); kpower = calloc(2*ngrid,sizeof(double)); 
	ipower_local = calloc(2*ngrid,sizeof(int)); ipower = calloc(2*ngrid,sizeof(int));
	...
	char pk_output[256]; 
	sprintf(pk_output, "%s.pk", output_fname); 
	get_pk_new(rank, numprocs, ngrid, TotNumPart, local_y_start_after_transpose, local_ny_after_transpose, BoxSize, dens_out, pk_output);

	rfftwnd_mpi(ip, 1, dens_in, work, FFTW_TRANSPOSED_ORDER);

	get_xi_new(ngrid, local_x_start, local_nx, dens_in, kpower_local, dpower_local,ipower_local); 



	/* Need to combine results from different slabs */
	MPI_Allreduce(dpower_local, dpower, 2*ngrid, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(kpower_local, kpower, 2*ngrid, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	MPI_Allreduce(ipower_local, ipower, 2*ngrid, MPI_INT, MPI_SUM, MPI_COMM_WORLD);


	int ibin; 
	if (rank==0) {
	  for (ibin=0; ibin<2*ngrid; ibin++)
	    if (ipower[ibin]>0) {
	      Pk[ibin] = dpower[ibin]/ipower[ibin];
	      kpower[ibin] /= ipower[ibin];
	    }
	    else {
	      Pk[ibin] = -1;
	      kpower[ibin] = -1;
	    }
	  /* 	    FILE *fpout = fopen("/home/jkwan/powerspectra/pk_fort.66_tsc.dat","w"); */
	  /* 	    sprintf(output_fname, "/home/jkwan/powerspectra/coyote/original/pk_dd_M%03d_%d_%.4f.dat",model_no,ngrid,header1.time); */
	    FILE *fpout = fopen(output_fname,"w");
/* 	    FILE *fpout = fopen(output_fname, "w");  */
/* 	    FILE *fpout = fopen("/home/jkwan/powerspectra/coyote/pk_fort.66_tsc_np_4.dat","w"); */
		for (ibin=0; ibin<2*ngrid; ibin++)
		    if (kpower[ibin]>0)
			fprintf(fpout, "%12.4e %12.4e \n",kpower[ibin]*BoxSize/(float)ngrid,Pk[ibin]);
		fflush(fpout);
		fclose(fpout); 
	    }
...
int butterworth(float flo, float fhi, int order, int num_time_pts, float deltafreq, double complex *fltr)
{
  ...
  int i, n, k; 
  //  fftw_complex one, s, sn; 
  double complex one, s, sn; 
  float scale, th, w1w2; 
  int num_freq_pts; 

  num_freq_pts = num_time_pts/2.0 + 1.0; 
  
  one = 1.0 + 0*I;
  w1w2 = flo*fhi; // I think this is the width
  scale = fabs(fhi - flo); // scale = w_c = radius of circle which poles lie on
  ...
  for (n = 1; n <= order; n++)
    {
      ...
      th = M_PI * (2. * n + order - 1)/ (2. * order); 
/*       sn.re = cos(th)*scale;  */
/*       sn.im = sin(th)*scale;  */
      double complex sn = cos(th)*scale + sin(th)*scale*I; 
      for (i= 1; i < num_freq_pts; i++)
	{
	  double complex s = 0 + (i*deltafreq - w1w2/(i*deltafreq))*I;

/* 	  fprintf(stderr, "%d %f %f %f %f\n", n, creal(dum), cimag(dum), creal(sn/(sn-s)), cimag(sn/(sn-s))); */
	  
  	  fltr[i] = fltr[i] *sn/(sn-s); 

	}
    }
  
/*   k = 1; */
/*   for (i = num_time_pts-1; i > (num_freq_pts-1); i--) */
/*     { */
/*       fltr[i] = conj(fltr[k]); */
/* /\*       fprintf(stderr, "%d %d \n", i, k);  *\/ */
/*       k = k + 1; */
...
/*   free(fltr);  */
  return(0); 
	...
	{
	    char filename[256]; 
	    sprintf(filename, "%s_%d", base, fileno);
	    FILE *fp = fopen(base, "r"); 
	    int count = 0, nread = 1; 
	    int i, j, k; 
    

	    for(count = *TotNumPart; count < (total_local_size+*TotNumPart); count++)
	    {
		nread = fscanf(fp, "%lf", &(dens_in[count])); 

		if (nread==EOF)
		{
		    fprintf(stderr, "End of file reached early!\n"); 
		    break; 
		}
	    }
	    *TotNumPart += count; 
...
int load_haloes(int *NumPartTotal, float BoxSize, struct particle_data **P)
{
    ...
    *NumPartTotal = 0; 
    ...
    FILE *fplist = fopen("/intrepid-fs0/users/jkwan/scratch/subhaloes/listofhaloes.dat", "r"); 
    
    int maxpart = 5e8; 
    *P = malloc(maxpart * sizeof (struct particle_data)); 

    while (nread != EOF)
    {
	nread = fscanf(fplist, "%s %d %d\n", filename, &haloid, &npart); 

	sprintf(fullfilename, "/intrepid-fs0/users/jkwan/scratch/subhaloes/%s", filename); 

	FILE *fp = fopen(fullfilename, "r"); 

	if ((*NumPartTotal+npart)> maxpart)
	{
	    fprintf(stderr, "Too many particles!\n"); 
	    exit(1); 
	}

	for (i = *NumPartTotal; i < (npart+*NumPartTotal); i++)
	{
	    long int ID; 
	    float any[7]; 
	    fread(&(any[0]), sizeof(float), 7, fp); 
	    fread(&ID, sizeof(long int), 1, fp); 
	    (*P)[i].Pos[0] = any[0]/BoxSize; 
	    (*P)[i].Pos[1] = any[2]/BoxSize; 
	    (*P)[i].Pos[2] = any[4]/BoxSize; 
	    (*P)[i].Vel[0] = any[1];
	    (*P)[i].Vel[1] = any[3];
	    (*P)[i].Vel[2] = any[5];
	}
...
/* 	       &((*P)[n].Pos[2]), &((*P)[n].Vel[0]),&((*P)[n].Vel[1]) ,  */
/* 	       &((*P)[n].Vel[2]),&any);  */
		fscanf(fp,"%f %f %f",&((*P)[n].Pos[0]),&((*P)[n].Pos[1]), &((*P)[n].Pos[2])); 
...
/* 		dens_in[index(i,j,k,n)] = 0.0;      */
	    ...
	    deltax = BoxSize/(float)n;
	    deltay = BoxSize/(float)n;
	    deltaz = BoxSize/(float)n;
	    int count = 0; 
...
/* 	znorm = P[l].Pos[2]/deltaz; */
		...
		xnorm = P[l].Pos[0]/(double)BoxSize; 
		ynorm = P[l].Pos[1]/(double)BoxSize; 
		znorm = P[l].Pos[2]/(double)BoxSize; 
...
/* 	k = my_modulo(n,k); */
		...
		int iw = i - local_x_start; 
		if (iw >= 0 && iw < local_nx)
		    dens_in[fftw_index_before(iw,j,k,n)] += 1.;
...
/* 	for (j = 0; j < n; j++) */
/* 	    for (k = 0; k < n; k++) */
/* 		fprintf(stderr, "%d %d %d %f\n", i, j, k, dens_in[fftw_index_before(i,j,k,n)]); */
...
/* 	k  = (int)(znorm+0.5); */
		...
		i  = ng*(P[l].Pos[0]/(double)BoxSize); 
		j  = ng*(P[l].Pos[1]/(double)BoxSize); 
		k  = ng*(P[l].Pos[2]/(double)BoxSize); 

/* 	if (i==(local_x_start-1) && fabs(ng*P[l].Pos[0]-local_x_start)<0.001) i=local_x_start; */
/* 	if (i==(local_x_start-1) && fabs(ng*P[l].Pos[0]-local_x_start)<0.001)  */
/* 	{ */
/* 	    fprintf(stderr, "%f %f %f\n", P[l].Pos[0], P[l].Pos[1], P[l].Pos[2]);  */
/* 	    i = local_x_start;  */
/* 	} */
/* 	if (fabs(ng*(P[l].Pos[0]/(float)BoxSize)-local_x_start)<0.001) i=local_x_start;  */
		/* 	/\*Distance of particle from cell center*\/ */

/* 	dx = xnorm-(double)i; */
/* 	dy = ynorm-(double)j; */ /*OLD*/
/* 	dz = znorm-(double)k; */

		dx = ng*(P[l].Pos[0]/(double)BoxSize)-(double)i;
		dy = ng*(P[l].Pos[1]/(double)BoxSize)-(double)j; 
		dz = ng*(P[l].Pos[2]/(double)BoxSize)-(double)k;
...
/* 	  hzm = 1-fabs(dz);  */
		...
		if(rank == 0 && ip== n-1 && im == 0 && nproc > 1)
		{
		    /*wrap around to top of box*/
		    dens_in[fftw_index_before(0 ,jp,kp,n)]  += hxm*hyp*hzp*mass;
		    dens_in[fftw_index_before(0 ,jp,km,n)]  += hxm*hyp*hzm*mass;
		    dens_in[fftw_index_before(0 ,jm,kp,n)]  += hxm*hym*hzp*mass;
		    dens_in[fftw_index_before(0 ,jm,km,n)]  += hxm*hym*hzm*mass;
		}

		if(rank == nproc-1 && ip==0 && im == n-1 && nproc > 1)
		{
		...
		if(rank == nproc-1 && ip==n-1 && im == 0 && nproc > 1)
		{

		    /*wrap around to bottom of box*/
		    dens_in[fftw_index_before(local_nx-1,jp,kp,n)]  += hxp*hyp*hzp*mass;
		    dens_in[fftw_index_before(local_nx-1,jp,km,n)]  += hxp*hyp*hzm*mass;
		    dens_in[fftw_index_before(local_nx-1,jm,kp,n)]  += hxp*hym*hzp*mass;
		    dens_in[fftw_index_before(local_nx-1,jm,km,n)]  += hxp*hym*hzm*mass;
		}
...
/* 	if (i >= local_x_start && i < (local_nx+local_x_start)) */
/* 	    count++;  */
		...
		if (ip >= -1 && ip <= local_nx && im >= -1 && im <= local_nx)  
		{
		    ...
		    if (im == -1 && ip == 0)
		    {
			/*Particle inside, neighbour outside, lower edge  i.e. ip = 0*/
	    ...
	    fprintf(stderr, "rank = %d, count = %d, %d, %d\n", rank, count, local_x_start, local_nx);
...
// need to check this, could be more robust:   dx    = ng*P[nn].r[0]-(ix+xlim[0]);
// Might need this:   if (ix==xlim[0]-1 && fabs(ng*P[nn].r[0]-xlim[0])<0.001) ix=xlim[0]; gets the particles on the edge of the slab
	dx    = ng*(P[l].Pos[0]/(double)BoxSize)-(double)i;
	dy    = ng*(P[l].Pos[1]/(double)BoxSize)-(double)j;
	dz    = ng*(P[l].Pos[2]/(double)BoxSize)-(double)k;
...
/* 	k  = (int)(znorm+0.5); */
	...
	/* 	dy=ynorm-floor(ynorm)-0.5; */
	/* 	dz=znorm-floor(znorm)-0.5; */
	/*Distance of particle from nearest FFT node*/
/* 	dx = xnorm-(double)i; */
/* 	dy = ynorm-(double)j; */
/* 	dz = znorm-(double)k; */
...
/*     MPI_Allreduce(&count &CountTotal, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD); */
    fprintf(stderr, "CountTotal= %d, rank = %d\n", CountTotal, rank);
/*     fprintf(stderr, "CountTotal= %d, rank = %d\n", CountTotal, rank); */
...
// need to check this, could be more robust:   dx    = ng*P[nn].r[0]-(ix+xlim[0]);
// Might need this:   if (ix==xlim[0]-1 && fabs(ng*P[nn].r[0]-xlim[0])<0.001) ix=xlim[0]; gets the particles on the edge of the slab
	dx    = ng*(P[l].Pos[0])-(double)i;
	dy    = ng*(P[l].Pos[1])-(double)j;
	dz    = ng*(P[l].Pos[2])-(double)k;
...
/* 	dz = znorm-(double)k; */
	...
	if (check_in_box >= -1 && check_in_box < local_nx+1 )  
	{
       ...
       /*Wrap around for period boundary conditions*/
	    if(j >= n) j=j-n;
	    if(k >= n) k=k-n;
		...
		if (nproc==1) //wrap around for periodic boundary conditions
		{
		    dens_in[fftw_index_before(n-1,jp,kp,n)]  += hx *hyp*hzp*mass;
		    dens_in[fftw_index_before(n-1,jp,k ,n)]  += hx *hyp*hz *mass;
		    dens_in[fftw_index_before(n-1,jp,km,n)]  += hx *hyp*hzm*mass;
		    dens_in[fftw_index_before(n-1,j ,kp,n)]  += hx *hy *hzp*mass;
		    dens_in[fftw_index_before(n-1,j ,k ,n)]  += hx *hy *hz *mass;
		    dens_in[fftw_index_before(n-1,j ,km,n)]  += hx *hy *hzm*mass;
		    dens_in[fftw_index_before(n-1,jm,kp,n)]  += hx *hym*hzp*mass;
		    dens_in[fftw_index_before(n-1,jm,k ,n)]  += hx *hym*hz *mass;
		    dens_in[fftw_index_before(n-1,jm,km,n)]  += hx *hym*hzm*mass;

		    
		    dens_in[fftw_index_before(n-2,jp,kp,n)]  += hxm *hyp*hzp*mass;
		    dens_in[fftw_index_before(n-2,jp,k ,n)]  += hxm *hyp*hz *mass;
		    dens_in[fftw_index_before(n-2,jp,km,n)]  += hxm *hyp*hzm*mass;
		    dens_in[fftw_index_before(n-2,j ,kp,n)]  += hxm *hy *hzp*mass;
		    dens_in[fftw_index_before(n-2,j ,k ,n)]  += hxm *hy *hz *mass;
		    dens_in[fftw_index_before(n-2,j ,km,n)]  += hxm *hy *hzm*mass;
		    dens_in[fftw_index_before(n-2,jm,kp,n)]  += hxm *hym*hzp*mass;
		    dens_in[fftw_index_before(n-2,jm,k ,n)]  += hxm *hym*hz *mass;
		    dens_in[fftw_index_before(n-2,jm,km,n)]  += hxm *hym*hzm*mass;
		...
		if (nproc==1)
		{
		    ...
		    dens_in[fftw_index_before(1,jp,kp,n)]  += hxm *hyp*hzp*mass;
		    dens_in[fftw_index_before(1,jp,k ,n)]  += hxm *hyp*hz *mass;
		    dens_in[fftw_index_before(1,jp,km,n)]  += hxm *hyp*hzm*mass;
		    dens_in[fftw_index_before(1,j ,kp,n)]  += hxm *hy *hzp*mass;
		    dens_in[fftw_index_before(1,j ,k ,n)]  += hxm *hy *hz *mass;
		    dens_in[fftw_index_before(1,j ,km,n)]  += hxm *hy *hzm*mass;
		    dens_in[fftw_index_before(1,jm,kp,n)]  += hxm *hym*hzp*mass;
		    dens_in[fftw_index_before(1,jm,k ,n)]  += hxm *hym*hz *mass;
		    dens_in[fftw_index_before(1,jm,km,n)]  += hxm *hym*hzm*mass;
		...
		if (nproc==1)
		{
		    dens_in[fftw_index_before(n-1,jp,kp,n)]  += hxm *hyp*hzp*mass;
		    dens_in[fftw_index_before(n-1,jp,k ,n)]  += hxm *hyp*hz *mass;
		    dens_in[fftw_index_before(n-1,jp,km,n)]  += hxm *hyp*hzm*mass;
		    dens_in[fftw_index_before(n-1,j ,kp,n)]  += hxm *hy *hzp*mass;
		    dens_in[fftw_index_before(n-1,j ,k ,n)]  += hxm *hy *hz *mass;
		    dens_in[fftw_index_before(n-1,j ,km,n)]  += hxm *hy *hzm*mass;
		    dens_in[fftw_index_before(n-1,jm,kp,n)]  += hxm *hym*hzp*mass;
		    dens_in[fftw_index_before(n-1,jm,k ,n)]  += hxm *hym*hz *mass;
		    dens_in[fftw_index_before(n-1,jm,km,n)]  += hxm *hym*hzm*mass;
		...
		{
		    dens_in[fftw_index_before(0,jp,kp,n)]  += hxp *hyp*hzp*mass;
		    dens_in[fftw_index_before(0,jp,k ,n)]  += hxp *hyp*hz *mass;
		    dens_in[fftw_index_before(0,jp,km,n)]  += hxp *hyp*hzm*mass;
		    dens_in[fftw_index_before(0,j ,kp,n)]  += hxp *hy *hzp*mass;
		    dens_in[fftw_index_before(0,j ,k ,n)]  += hxp *hy *hz *mass;
		    dens_in[fftw_index_before(0,j ,km,n)]  += hxp *hy *hzm*mass;
		    dens_in[fftw_index_before(0,jm,kp,n)]  += hxp *hym*hzp*mass;
		    dens_in[fftw_index_before(0,jm,k ,n)]  += hxp *hym*hz *mass;
		    dens_in[fftw_index_before(0,jm,km,n)]  += hxp *hym*hzm*mass;
...
/* 	    count+=9; */
	...
	else if(rank == 0 && i == n && nproc > 1)
	{
	    ...
	    dens_in[fftw_index_before(1 ,jp,kp,n)]  += hxp *hyp*hzp*mass;
	    dens_in[fftw_index_before(1 ,jp,k ,n)]  += hxp *hyp*hz *mass;
	    dens_in[fftw_index_before(1 ,jp,km,n)]  += hxp *hyp*hzm*mass;
	    dens_in[fftw_index_before(1 ,j ,kp,n)]  += hxp *hy *hzp*mass;
	    dens_in[fftw_index_before(1 ,j ,k ,n)]  += hxp *hy *hz *mass;
	    dens_in[fftw_index_before(1 ,j ,km,n)]  += hxp *hy *hzm*mass;
	    dens_in[fftw_index_before(1 ,jm,kp,n)]  += hxp *hym*hzp*mass;
	    dens_in[fftw_index_before(1 ,jm,k ,n)]  += hxp *hym*hz *mass;
	    dens_in[fftw_index_before(1 ,jm,km,n)]  += hxp *hym*hzm*mass;
...
/* 	    count+=9; */
	...
	else if(rank == nproc-1 && i==0 && nproc > 1)
	{
	    ...
	    /*wrap around to top of box*/
	    dens_in[fftw_index_before(local_nx-1,jp,kp,n)]  += hxm *hyp*hzp*mass;
	    dens_in[fftw_index_before(local_nx-1,jp,k ,n)]  += hxm *hyp*hz *mass;
	    dens_in[fftw_index_before(local_nx-1,jp,km,n)]  += hxm *hyp*hzm*mass;
	    dens_in[fftw_index_before(local_nx-1,j ,kp,n)]  += hxm *hy *hzp*mass;
	    dens_in[fftw_index_before(local_nx-1,j ,k ,n)]  += hxm *hy *hz *mass;
	    dens_in[fftw_index_before(local_nx-1,j ,km,n)]  += hxm *hy *hzm*mass;
	    dens_in[fftw_index_before(local_nx-1,jm,kp,n)]  += hxm *hym*hzp*mass;
	    dens_in[fftw_index_before(local_nx-1,jm,k ,n)]  += hxm *hym*hz *mass;
	    dens_in[fftw_index_before(local_nx-1,jm,km,n)]  += hxm *hym*hzm*mass;
/* 	    count+=9; */
	}
	else if(rank == nproc-1 && i==-1 && nproc > 1)
	{
	    if(j >= n) j=j-n;
	    if(k >= n) k=k-n;
	    ...
	    dens_in[fftw_index_before(local_nx-2,jp,kp,n)]  += hxm *hyp*hzp*mass;
	    dens_in[fftw_index_before(local_nx-2,jp,k ,n)]  += hxm *hyp*hz *mass;
	    dens_in[fftw_index_before(local_nx-2,jp,km,n)]  += hxm *hyp*hzm*mass;
	    dens_in[fftw_index_before(local_nx-2,j ,kp,n)]  += hxm *hy *hzp*mass;
	    dens_in[fftw_index_before(local_nx-2,j ,k ,n)]  += hxm *hy *hz *mass;
	    dens_in[fftw_index_before(local_nx-2,j ,km,n)]  += hxm *hy *hzm*mass;
	    dens_in[fftw_index_before(local_nx-2,jm,kp,n)]  += hxm *hym*hzp*mass;
	    dens_in[fftw_index_before(local_nx-2,jm,k ,n)]  += hxm *hym*hz *mass;
	    dens_in[fftw_index_before(local_nx-2,jm,km,n)]  += hxm *hym*hzm*mass;
...
int finish_grid(int n, int local_nx, int TotNumPart, float BoxSize, fftw_real *dens_in)
{
    ...
    double rhobar = (double)TotNumPart/((double)n*(double)n*(double)n);
/*       float mdens = (float)TotNumPart/((float)n*(float)n*(float)n); */
/*       float rhobar = 1./((float)n*(float)n*(float)n)/mdens;  */
/*       rhobar = rhobar * rhobar;  */
	...
	for (j = 0; j < n; j++)
	    for (k = 0; k < n; k++)
	    {
		double delta = dens_in[fftw_index_before(i,j,k,n)]/rhobar-(double)1.;  
	    }
/* 		dens_in[fftw_index_before(i,j,k,n)] = dens_in[fftw_index_before(i,j,k,n)]-(double)rhobar.;   */
...
void	calc_pk(int me, int nproc, int Ng, int Np, rfftwnd_mpi_plan fftfplan, struct particle_data *P)
/* Calculate P(k) */
{
fftw_real 	*ddat,*sdat,*work=NULL;
fftw_complex 	*dgrid;
int		xlim[2],ylim[2],fftsize,ibin=0;
int		leftpart,rightpart,*lcnt,*acnt;
float		k2,d2,kmax2,scale,dx,dy,dz,*pk;
double		ng,mdens,totm,B,tPi2,wk[3];
...
MPI_Status	status;
 int count = 0; 
  ...
  ddat = (fftw_real *)malloc( fftsize*sizeof(fftw_real));
  dgrid= (fftw_complex *) &ddat[0];

  /* Fill the density grid. */

  for (ix=0; ix<fftsize; ix++) ddat[ix]=0;

  for (nn=0; nn<Np; nn++) {
    ix = ng*P[nn].Pos[0];
    if (ix==xlim[0]-1 && fabs(ng*P[nn].Pos[0]-xlim[0])<0.001)
    {
	ix=xlim[0];
	count++; 
    }
    ...
    if (ix>=xlim[0] && ix<xlim[0]+xlim[1]) {
      ...
      dz    = ng*P[nn].Pos[2]-iz;
/* #ifdef	CIC */
      ddat[(Ng*Ng2* ix  +Ng2* iy  + iz  )] += (1.0-dx)*(1.0-dy)*(1.0-dz);
...
/*       if (ddat[(Ng*Ng2* ix + Ng2*iy +iz)]< 0) */
/* 	  fprintf(stderr, "Help! Density is negative: %d %d %d %lf %f %f %f %f\n", ix+xlim[0],iy,iz,    ddat[(Ng*Ng2* ix + Ng2*iy +iz)],     (1.0-dx), (1.0-dy), (1.0-dz), P[nn].Pos[0]);  */
/*       else if (ddat[(Ng*Ng2* ii[0] + Ng2*iy +iz)]< 0) */
/* 	  fprintf(stderr, "Help! Density is negative: %d %d %d %lf %f %f %f %f\n", ii[0]+xlim[0],iy,iz, ddat[(Ng*Ng2* ii[0] + Ng2*iy +iz)],  dx, (1.0-dy), (1.0-dz),P[nn].Pos[0]);  */
/*       else if (ddat[(Ng*Ng2* ix + Ng2*ii[1] +iz)]< 0) */
/* 	  fprintf(stderr, "Help! Density is negative: %d %d %d %lf %f %f %f %f\n", ix+xlim[0],ii[1],iz, ddat[(Ng*Ng2* ix + Ng2*ii[1] +iz)],  (1.0-dx), dy, (1.0-dz),P[nn].Pos[0]);  */
/*       else if (ddat[(Ng*Ng2* ix  +Ng2* iy  +ii[2])]<0) */
/* 	  fprintf(stderr, "Help! Density is negative: %d %d %d %lf %f %f %f %f\n", ix+xlim[0],iy,ii[2], ddat[(Ng*Ng2* ix  +Ng2* iy  +ii[2])],(1.0-dx), (1.0-dy), dz,P[nn].Pos[0]);  */
/*       else if (ddat[(Ng*Ng2*ii[0]+Ng2*ii[1]+ iz  )]<0) */
/* 	  fprintf(stderr, "Help! Density is negative: %d %d %d %lf %f %f %f %f\n", ii[0]+xlim[0],ii[1],iz, ddat[(Ng*Ng2*ii[0]+Ng2*ii[1]+ iz  )], dx, dy, (1.0-dz),P[nn].Pos[0]);  */
/*       else if(ddat[(Ng*Ng2*ii[0]+Ng2* iy  +ii[2])]<0) */
/* 	  fprintf(stderr, "Help! Density is negative: %d %d %d %lf %f %f %f %f\n", ii[0]+xlim[0],iy,ii[2], ddat[(Ng*Ng2*ii[0]+Ng2* iy  +ii[2])], dx, (1.0-dy), dz,P[nn].Pos[0]);  */
/*       else if(ddat[(Ng*Ng2* ix  +Ng2*ii[1]+ii[2])]<0) */
/* 	  fprintf(stderr, "Help! Density is negative: %d %d %d %lf %f %f %f %f\n", ix+xlim[0],ii[1],ii[2], ddat[(Ng*Ng2* ix  +Ng2*ii[1]+ii[2])], (1.0-dx), dy, dz,P[nn].Pos[0]);  */
/*       else if(ddat[(Ng*Ng2*ii[0]+Ng2*ii[1]+ii[2])] < 0) */
/* 	  fprintf(stderr, "Help! Density is negative: %d %d %d %lf %f %f %f %f\n", ii[0]+xlim[0],ii[1],ii[2], ddat[(Ng*Ng2*ii[0]+Ng2*ii[1]+ii[2])], dx, dy, dz,P[nn].Pos[0]);  */
/* #else */
/*       ddat[(Ng*Ng2* ix  +Ng2* iy  + iz  )] += 1; */
/* #endif */
    }
/*     else { */
/*       printf("WARNING: %d has particle %d, ix=%d, not assigned to grid.\n", */
/*         me,nn,ix); */
/*       printf("P.Pos[0]=%e, xlim=(%d,%d).\n",P[nn].Pos[0],xlim[0],xlim[1]); */
...
/*     } */
  ...
  sdat      = (fftw_real *)malloc(Ng*Ng2*sizeof(fftw_real));
  for (ix=0; ix<Ng*Ng2; ix++) sdat[ix]=0;
  rightpart = (me + 1)%nproc;
  leftpart  = (me - 1); if (leftpart<0) leftpart+=nproc;

  if (me&1) {
    MPI_Ssend(&ddat[Ng*Ng2*xlim[1]],Ng*Ng2*sizeof(fftw_real),
              MPI_BYTE,rightpart,me,MPI_COMM_WORLD);
    MPI_Recv(sdat,Ng*Ng2*sizeof(fftw_real),
             MPI_BYTE,leftpart,leftpart,MPI_COMM_WORLD,&status);
  }
  else {
    MPI_Recv(sdat,Ng*Ng2*sizeof(fftw_real),
             MPI_BYTE,leftpart,leftpart,MPI_COMM_WORLD,&status);
  ...
  for (iy=0; iy<Ng; iy++)
    for (iz=0; iz<Ng; iz++)
      ddat[(Ng*Ng2*0+Ng2*iy+iz)] += sdat[Ng2*iy+iz];

  free(sdat);

  char specialfout[256];
  sprintf(specialfout, "/home/jkwan/powerspectra/cic_density_check_pk_mpi_%d.dat",me);
  FILE* fp_check = fopen(specialfout, "w");
  for(ix=0; ix < xlim[1]; ix++)
      for(iy=0; iy < Ng; iy++)
	  for(iz=0; iz < Ng; iz++)
	  {
	      fprintf(fp_check, "%d %d %d %lf \n", ix+xlim[0], iy, iz, ddat[(Ng*Ng2* ix  +Ng2* iy  + iz  )]);
	  }
  fprintf(fp_check, "\n"); 
  fclose(fp_check);

  fprintf(stderr, "pk_mpi.c: rank = %d, count = %d\n", me, count); 
...
/*   lcnt = mymalloc(  Nbin*sizeof(int),   "calc_pk sdat"); */
/*   acnt = mymalloc(  Nbin*sizeof(int),   "calc_pk acnt"); */
/*   pk   = mymalloc(2*Nbin*sizeof(float), "calc_pk pk"); */
/*   for (ix=0; ix<  Nbin; ix++) lcnt[ix]=acnt[ix]=0; */
/*   for (ix=0; ix<2*Nbin; ix++) pk[ix]=0; */
...
/*   MPI_Reduce(pk,pkall,2*Nbin,MPI_FLOAT,MPI_SUM,0,MPI_COMM_WORLD); */
...
/*       float fftw_norm = powf(ngrid/BoxSize,3)/powf(ngrid,3.);   original */
    float fftw_norm = powf(BoxSize/(float)ngrid,3)/powf((float)ngrid,3.);
    double totm = (double)NumPart;
    double mdens= totm/pow((double)ngrid,3.0);
    double scale = 1.0/pow((double)ngrid,3.0)/mdens;
/*     double fftw_norm = 1.0/pow((double)ngrid,6.0)/mdens/mdens; */
    scale = scale*scale*pow((double)BoxSize,3.0);
...
/*     float fftw_norm = 1.; */
    ...
    int Nbin = 1000; 
    double k2; 
    double tPi2 = 2.*2.*M_PI*M_PI; 
    double kmax2= 2*M_PI*(ngrid/2) * 0.8;
    kmax2 = kmax2*kmax2;
    int ii[3];
    double power; 
...
/* 					(float)pow(k,2))); */
		if (i>ngrid/2)
		    ...
		    ii[0] = i;
		if ((j+local_y_start)>ngrid/2)
		    ii[1] = (j+local_y_start) - ngrid;
		else
		    ii[1] = j+local_y_start;
		if (k>ngrid/2)
		    ii[2] = k - ngrid;
		else
		    ii[2] = k;

/* 		k2 = (double)i*(double)i+(double)(j+local_y_start)*(double)(j+local_y_start)+(double)k*(double)k;  */
...
/* 		c_im(dens_out[fftw_index_after(j,i,k,ngrid)]) = 0.0; */
		...
		power = pow(c_re(dens_out[fftw_index_after(j,i,k,ngrid)]),2)+
		    pow(c_im(dens_out[fftw_index_after(j,i,k,ngrid)]),2);


		k2 = (double)ii[0]*(double)ii[0]+(double)ii[1]*(double)ii[1]+(double)ii[2]*(double)ii[2]; 
 		k2 *= tPi2;
		ks = Nbin*log(k2/tPi2)/(log(kmax2/tPi2))+0.49999;
		ks = sqrt(k2/tPi2)/1.0;
...
/* 		kz  = M_PI*(double)k/(double)ngrid; */
		...
		kx  = M_PI*(double)ii[0]/(double)ngrid; 
		ky  = M_PI*(double)ii[1]/(double)ngrid;
		kz  = M_PI*(double)ii[2]/(double)ngrid;
...
/* 		fcorrect = 1.; */
		...
		if(k2 > 0 && k2 < kmax2)  /*A factor of dk has been taken out */
		{
/* 		    dpower[ks] += fcorrect*(double)c_re(dens_out[fftw_index_after(j,i,k,ngrid)])*fftw_norm; */
/* 		    dpower[ks] += fcorrect*(double)c_re(dens_out[fftw_index_after(j,i,k,ngrid)])*scale; */
		    dpower[ks] += fcorrect*power*fftw_norm;
		    ipower[ks] += 1;
		    kpower[ks] += sqrt(k2); 
		}
	    }  
...
int get_pk_new(int rank, int nproc, int n, double TotNumPart, int local_y_start_after_transpose, int local_ny_after_transpose, float BoxSize, fftw_complex*dgrid, char filename[]) 
{
    ...
    double *pk   = malloc(2*Nbin*sizeof(double));  
    double *pkall= malloc(2*Nbin*sizeof(double));
...
/* 	fprintf(stderr, "%d %d %d %f \n", ii[0], ii[1], ii[2], k2);  */
		...
		int    ip   = Ng*(Ng/2+1)*(iy-local_y_start_after_transpose)+(Ng/2+1)*ix+iz;

		if (k2>0 && k2<kmax2) {

/* 			    ibin = ngrid*log(k2/tPi2)/(log(kmax2/tPi2))+0.49999; */
...
/* /\* 		    d2  *= (float)k2*sqrt((float)k2)/2./M_PI/M_PI; *\/ */
/* 		    ibin = 4.0*sqrt((float)k2/tPi2)/1.0; //this is much smoother but use 2-> 4 for L runs */
/* /\* 		    ibin = Nbin*log(k2/tPi2)/(log(kmax2/tPi2))+0.49999;  *\/ */
		    if(ibin>=0 && ibin<Nbin) {
			if (iz>0 || (iz==0 && ii[0]>0) || (iz==0 && ii[0]==0 && ii[1]>0)) {
			    pk[2*ibin+0] += sqrt(k2);
			    pk[2*ibin+1] += dgrid[ip].re;
			    lcnt[ibin]++;
...
/*     char file_counts[256];  */
/*     sprintf(file_counts, "/home/jkwan/powerspectra/lcounts_mine_%d.dat",rank);  */
/*     FILE *fplcnt = fopen(file_counts, "w");  */
/*     for (ibin=0; ibin<Nbin; ibin++) */
/* 	fprintf(fplcnt, "%d\n", lcnt[ibin]);  */
/*     fclose(fplcnt);  */
    MPI_Reduce(lcnt,acnt, Nbin,MPI_INT  ,MPI_SUM,0,MPI_COMM_WORLD);
	...
	for (ibin=0; ibin<Nbin; ibin++)
	    if (acnt[ibin]>0) {
		pkall[2*ibin+0] /= (double)acnt[ibin];
		pkall[2*ibin+1] /= (double)acnt[ibin];
	    }
	    else {
		pkall[2*ibin+0] = -1;
		pkall[2*ibin+1] = -1;
...
/* int get_pk_new(int rank, int nproc, int n, int NumPart, int local_y_start_after_transpose, int local_ny_after_transpose, float BoxSize, fftw_complex*dgrid, double *kmodes, double *pk, int *lcnt)  */
/* { */
/*     int i, j, k,l, index, flag = 0; */
/*     float rhobar; */
/*     double xnorm,ynorm,znorm, deltax, deltay, deltaz, dx, dy, dz; */
...
/*     for (ix=0; ix<  Nbin; ix++) lcnt[ix]=0; */
/*     for (ix=0; ix<  Nbin; ix++) pk[ix]=0; */
/*     for (ix=0; ix<  Nbin; ix++) kmodes[ix]=0; */
...
/* /\* 		pkall[2*ibin+1] = -1; *\/ */
/* /\* 	    } *\/ */
/* /\* 	FILE *fpout = fopen("/home/jkwan/powerspectra/pk_fort.66.dat","w"); *\/ */
/* /\* 	for (ibin=0; ibin<Nbin; ibin++) *\/ */
/* /\* 	    if (pkall[2*ibin+0]>0) *\/ */
/* /\* 		fprintf(fpout, "%12.4e %12.4e %d\n",pkall[2*ibin+0]/BoxSize,pkall[2*ibin+1], acnt[ibin]); *\/ */
/* /\* 	fflush(fpout); *\/ */
/* /\* 	fclose(fpout);  *\/ */
/* /\*     } *\/ */
...
int get_pk_new_redshift(int rank, int nproc, int n, int nmu, int NumPart, int local_y_start_after_transpose, int local_ny_after_transpose, float BoxSize, int los, fftw_complex*dgrid, double *kmodes, double *kmodes_para, double *kmodes_perp, double *pk, double *pk_mono, double *pk_quad, int *lcnt, int *lcnt_para, int *lcnt_perp, int *lcnt_multipole) 
{
    ...
    float		k2,k_para,k_perp,d2,kmax2;
...
/*     float *pkall= malloc(2*Nbin*sizeof(float)); */
    ...
    for (ix=0; ix<  Nbin*nmu; ix++) lcnt[ix]=0;
    for (ix=0; ix<  Nbin*nmu; ix++) pk[ix]=0;
    

    for (ix=0; ix<  Nbin; ix++) lcnt_multipole[ix]=0;
    for (ix=0; ix<  Nbin; ix++) lcnt_para[ix]=0;
    for (ix=0; ix<  Nbin; ix++) lcnt_perp[ix]=0;
    for (ix=0; ix<  Nbin; ix++) kmodes[ix]=0;
    for (ix=0; ix<  Nbin; ix++) kmodes_para[ix]=0;
    for (ix=0; ix<  Nbin; ix++) kmodes_perp[ix]=0;
    for (ix=0; ix<  Nbin; ix++) pk_mono[ix]=0;
    for (ix=0; ix<  Nbin; ix++) pk_quad[ix]=0;
...
int get_xi_new(int ngrid, int local_x_start, int local_nx, fftw_real *dens_in, double *kpower_local, double *dpower_local, int *ipower_local)
{
    double k2; 
    int ii[3],ibin; 
    double tPi2 = (2.*M_PI)*(2.*M_PI);
    double kmax2= 2*M_PI*((double)ngrid/2.) * 0.8;
    kmax2= kmax2*kmax2;
/*     float mdens= TotNumPart/pow((double)ngrid,3.0); */
/* 	    float scale = 1.0/pow((double)ngrid,3.0)/mdens; */
/* 	    scale = scale*scale; */



    int ix, iy, iz; 
    for (ix=local_x_start; ix<(local_x_start+local_nx); ix++)
	for (iy=0; iy<ngrid; iy++)
		...
		k2  = ii[0]*ii[0]+
		    ii[1]*ii[1]+
		    ii[2]*ii[2];
		int index   = ngrid*(2*(ngrid/2+1))*(ix-local_x_start)+(2*(ngrid/2+1))*iy+iz;
		if (k2>0 && k2<kmax2) {
...
/* 		    ibin = 2*sqrt(k2/tPi2)/1.0; //original */
		    ibin = sqrt(k2)/1.0;
/* 		    if(ibin>=0 && ibin<2*n) { */
		    if(ibin>=0 && ibin<ngrid) {
			if (iz>0 || (iz==0 && ii[0]>0) || (iz==0 && ii[0]==0 && ii[1]>0)) {
			    kpower_local[ibin] += sqrt(k2);
			    dpower_local[ibin] += dens_in[index]; 
			    ipower_local[ibin]++;
			}
		    }
	...
	for (iy=0; iy<ngrid; iy++)
		...
		if (k2>0 && k2<kmax2) {
			...
			switch (los)
			{
				...
				ibin_para = sqrt(ii[0]*ii[0]*1.0)/1.0;
				ibin_perp = sqrt(ii[1]*ii[1]*1.0+ii[2]*ii[2]*1.0)/1.0;
				k_para = sqrt(ii[0]*ii[0]); 
				k_perp = sqrt(ii[1]*ii[1]+ii[2]*ii[2]);
				legendre2 = 2.5*(3.*(float)ii[0]*(float)ii[0]/k2-1.);
				break;
			    case 1:
				ibin_para = sqrt(ii[1]*ii[1]*1.0)/1.0;
				ibin_perp = sqrt(ii[0]*ii[0]*1.0+ii[2]*ii[2]*1.0)/1.0;
				k_para = sqrt(ii[1]*ii[1]); 
				k_perp = sqrt(ii[0]*ii[0]+ii[2]*ii[2]);
				legendre2 = 2.5*(3.*(float)ii[1]*(float)ii[1]/k2-1.);
				break;
			    case 2:
				ibin_para = sqrt(ii[2]*ii[2]*1.0)/1.0;
				ibin_perp = sqrt(ii[0]*ii[0]*1.0+ii[1]*ii[1]*1.0)/1.0;
				k_para = sqrt(ii[2]*ii[2]); 
				k_perp = sqrt(ii[0]*ii[0]+ii[1]*ii[1]);
				legendre2 = 2.5*(3.*(float)ii[2]*(float)ii[2]/k2-1.);
				break;
			    default:
				ibin_para = 0; ibin_perp = 0; 
				break;
			...
			pk[ibin_para+ibin_perp*nmu] += dens_in[index];
			kmodes_para[ibin_para]+=k_para; 
			kmodes_perp[ibin_perp]+=k_perp; 
			lcnt_para[ibin_para]++;
			lcnt_perp[ibin_perp]++;
			...
			kmodes[ibin] += sqrt(k2);
			pk_mono[ibin] += dens_in[index];
			pk_quad[ibin] += dens_in[index]*legendre2;
			lcnt_multipole[ibin]++; 
...
/*     float scale = 1.0/pow((double)Ng,3.0)/mdens; */
    float scale = 1.0/pow((double)Ng,3.0);
    scale = scale*scale;
    
    int Nbin = Ng; 
    int ibin=0; 
    double complex fltrx, fltry, fltrz; 
	...
	for (ix=0; ix<Ng; ix++)
		...
		k2  = ii[0]*ii[0]+ ii[1]*ii[1]+ ii[2]*ii[2];
		k2 *= tPi2;
		ip   = Ng*(Ng/2+1)*(iy-local_y_start_after_transpose)+(Ng/2+1)*ix+iz; 
/* 	fprintf(stderr, "%d %d %d %f \n", ii[0], ii[1], ii[2], k2);  */
		if (k2 > 0 && k2<kmax2) {
...
int get_pk_legendre(int ngrid, int NumPart, int local_ny, int local_y_start, float BoxSize, fftw_complex *dens_out, int los, double *dpower_mono, double *dpower_quad, int *ipower)
{
    int i,j,k, l,ks, *kmod;
    float ks_min, ks_max, dk;
    double kx, ky, kz, kR, kmodulus, fcorrect,scorrect, legendre2;
	...
	for (i = 0; i < ngrid; ++i)
		...
		fcorrect = 1./pow(fcorrect,6.);
                ...
                switch (los)
                  {
                  case 0:
                    legendre2 = 1.25*(3*(float)i/(float)ks*(float)i/(float)ks-1);
                    break;
                  case 1:
                    legendre2 = 1.25*(3*(float)j/(float)ks*(float)j/(float)ks-1);
                    break;
                  case 2:
                    legendre2 = 1.25*(3*(float)k/(float)ks*(float)k/(float)ks-1);
                    break;
                  default:
		    legendre2=1.; 
                    break;
		...
		{
		    dpower_mono[ks] += fcorrect*(double)c_re(dens_out[fftw_index_after(j,i,k,ngrid)])*fftw_norm;
		    dpower_quad[ks] += fcorrect*(double)c_re(dens_out[fftw_index_after(j,i,k,ngrid)])*fftw_norm*legendre2;
		    ipower[ks] += 1;
		}
	    }  
		     ...
		     fftw_complex *dens_out, int los, double *dpower, int *ipower)
{
	...
	for (i = 0; i < ngrid; ++i)
		...
		if(ks > 0 && ks < ngrid/2.)  /*A factor of dk has been taken out */
		{
			...
			case 0:
			    mu = i;
			    ks = (int)floor(sqrt(j*j+k*k));
			    break;
			case 1:
			    mu = j;
			    ks = (int)floor(sqrt(i*i+k*k));
			    break;
			case 2:
			    mu = k;
			    ks = (int)floor(sqrt(i*i+j*j));
			    break;
			default:
			    mu = 0;
			    break;
...
/*     else */
    ...
    struct hacc_particle_data dummy; 

    /*Need to allocate particle data P*/
    /*Guess a size for P*/
    int maxpart = 2048.*2048.*2048./500.; 
...
/*     }  */
    ...
    *NumPart = 0; 
    ...
    {
/* 	nread = fread(&dummy, sizeof(struct hacc_particle_data), 1, fd); */
	nread = fread(&any, sizeof(float), 7, fd); 
/* 	fprintf(stderr, "%f %f %f %f %f %f %f\n", any[0], any[1], any[2], any[3], any[4], any[5], any[6]);  */
	nread = fread(&ID, sizeof(long int), 1, fd); 
/* 	fprintf(stderr, "ID=%ld\n", ID);  */
/* 	fprintf(stderr, "%lf %lf %lf %lf %ld\n", dummy.data[0], dummy.data[1], dummy.data[2], dummy.data[3], dummy.ID);  */
	if (!nread) break;
...
int load_snapshot(int nfiles, int fileno, char input_fname[], int *NumPart, int *TotNumPart, struct particle_data **P, struct io_header_1 *header1)
{
  ...
  (*TotNumPart) = (*header1).npartTotal[1];
...
/* 	  fprintf(stderr, "%f %f %f \n", (*P)[pc_new].Pos[0]/(*header1).BoxSize*16., (*P)[pc_new].Pos[1]/(*header1).BoxSize*16., (*P)[pc_new].Pos[2]/(*header1).BoxSize*16.);  */
/* 	  fprintf(stderr, "P[%d].Pos[0] = %f;\n", pc_new,(*P)[pc_new].Pos[0]/(*header1).BoxSize);  */
/* 	  fprintf(stderr, "P[%d].Pos[1] = %f;\n", pc_new,(*P)[pc_new].Pos[1]/(*header1).BoxSize);  */
/* 	  fprintf(stderr, "P[%d].Pos[2] = %f;\n", pc_new,(*P)[pc_new].Pos[2]/(*header1).BoxSize);  */
/* 	  (*P)[pc_new].Pos[0]/=1000.; */
/* 	  (*P)[pc_new].Pos[1]/=1000.; */
/* 	  (*P)[pc_new].Pos[2]/=1000.; */
  ...
  for (i = 0; i < NumPart; i++)
    {
      ...
      P[i].Pos[los] += (1.+z)*P[i].Vel[los]/H_z/100./BoxSize;  //The boxsize is there because the positions have been scaled to an unit cube.  
      if (P[i].Pos[los] > 1.)
	  P[i].Pos[los]-=1.;
      else if(P[i].Pos[los] < 0.)
	  P[i].Pos[los]+=1.;
...
int read_parameters(int model_no, float *Om, float *ns, float *w, float *sigma8, float *h)
{
  FILE *fp = fopen("/global/u1/j/jkwan/halo_catalogues/modelparams.dat","r"); 
  ...
  for (i = 0; i < 38; i++)
    {
      ...
      fscanf(fp, "%s %f %f %f %f %f %f %f %f", &(model[0]), &any[0], &any[1], &any[2], &any[3], &any[4], &any[5], &any[6], &any[7]); 
/*       fscanf(fp, "%s %f %f %f %f %f %f %f %f %f", &model, &Om[i],  &any[0], &ns[i],   &w[i], &sigma8[i], &h[i], &knl[i], &any[4]);  */
      if (i==model_no)
	{
	  *Om = any[0]; 
	  *ns = any[2]; 
