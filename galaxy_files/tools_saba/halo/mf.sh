...
if [ $ftype == 3 ]
then
  #python /project/projectdirs/hacc/PDACS/pdacs-test/tools/halo/GenericIOPrint.py --input $cmfile --outFile $cfile --noRank 0
  ${tools_dir}/Frontend_reader/GenericIOPrint $cmfile > $cfile
fi

propfile="${tools_dir}/working/${tmpfile}"

#python /project/projectdirs/hacc/PDACS/pdacs-test/tools/halo/GenericIOPrint.py --input $value --outFile $propfile --noRank 0
