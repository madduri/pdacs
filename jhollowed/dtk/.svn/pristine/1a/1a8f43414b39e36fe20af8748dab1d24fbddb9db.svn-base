...
namespace dtk{
  ...
  template <class T, class U>
  void reorder(T* data, int size, U* srt);
  template <class T, class U>
  void reorder(std::vector<T>& data,U* srt); // Takes an array/vector and reorders 
 ...
 template<class T,class U>
  void reorder(T* data, int64_t size, U* srt){
     T* tmp = new T[size];
    for(U i=0;i<size;++i){
      tmp[i]=data[srt[i]];
    }
  ...
  template<class T,class U>
  void reorder(std::vector<T>& data,U* srt){
  ...
  template<class T,class U>
  void reorder(T* data,  U* srt, int64_t size){
