...
pushd /nfs/software/galaxy-globus/swift_scripts/qc2 > /dev/null 2>&1
...
/usr/bin/time swift -sites.file sites.xml -tc.file tc.data -config cf -cdm.file fs.alldirect qc2.swift -level=$LEVEL $SWIFTARGS >> $LOG 2>&1
