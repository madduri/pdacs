...
#   os.system("/global/project/projectdirs/hacc/PDACS/pdacs-test/tools/halo/GenericIOPrint.py propinFile propFile 0")
...
#   propinFile=cur.fetchone()
#   os.system("/global/project/projectdirs/hacc/PDACS/pdacs-test/tools/halo/GenericIOPrint.py propinFile cMFile 0")

print zseek, hubble, Omegam, ns, sigma8, w0
...
if options.fType == 1:
	...
	print "mass Msun/h= ", '%le %le' %(FOFbinstart, FOFbinend)
        ...
        print >>file, "#FOF_Mass_Msun_h\tclusters\tdn_dln_M_h_Mpc_three\tfrac_err\tone_sigma_M\tf_sigma\tfsigma_fit\n"
...
if options.fType == 2 or options.fType == 3:
	...
	print "mass Msun/h= ", '%le %le' %(SObinstart, SObinend)
        ...
        if options.fType == 2: 
	   file= open(outputfile,'w')
           print>> file, "#SO_Mass_Msun_h\tclusters\tdn_dln_M_h_Mpc_three\tfrac_err\tone_sigma_M\tf_sigma\n"
...
if options.fType == 3:
	...
	file= open(outputfile,'w')
        print>>file, "#SO_Mass_Msun_h\tclusters\tcmean\tfrac_err\tvariance\n"
        for i in range(0,massbins-1):  
...
filelist = [ f for f in os.listdir("/global/project/projectdirs/hacc/PDACS/working/") if f.startswith("dataset") ]
for f in filelist:
    os.remove("/global/project/projectdirs/hacc/PDACS/working/" + f)
